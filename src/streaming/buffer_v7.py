...
@dataclass
class BufferV7Config:
    """Configuration for buffer_v7 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class BufferV7(nn.Module):
    """Neural network module for buffer_v7 v2.
    ...
    def __init__(self, config: Optional[BufferV7Config] = None):
        super().__init__()
        self.config = config or BufferV7Config()
        c = self.config
        ...
        logger.info("Initialized BufferV7 v2 with %d params",
