"""ProcessorV22: processor_v22 module v1.
...
@dataclass
class ProcessorV22Config:
    """Configuration for processor_v22 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class ProcessorV22(nn.Module):
    """Neural network module for processor_v22 v1.
    ...
    def __init__(self, config: Optional[ProcessorV22Config] = None):
        super().__init__()
        self.config = config or ProcessorV22Config()
        c = self.config
        ...
        logger.info("Initialized ProcessorV22 v1 with %d params",
