"""BufferV22: buffer_v22 module v2.
...
@dataclass
class BufferV22Config:
    """Configuration for buffer_v22 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class BufferV22(nn.Module):
    """Neural network module for buffer_v22 v2.
    ...
    def __init__(self, config: Optional[BufferV22Config] = None):
        super().__init__()
        self.config = config or BufferV22Config()
        c = self.config
        ...
        logger.info("Initialized BufferV22 v2 with %d params",
