"""RoutesV20: routes_v20 module v1.
...
@dataclass
class RoutesV20Config:
    """Configuration for routes_v20 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class RoutesV20(nn.Module):
    """Neural network module for routes_v20 v1.
    ...
    def __init__(self, config: Optional[RoutesV20Config] = None):
        super().__init__()
        self.config = config or RoutesV20Config()
        c = self.config
        ...
        logger.info("Initialized RoutesV20 v1 with %d params",
