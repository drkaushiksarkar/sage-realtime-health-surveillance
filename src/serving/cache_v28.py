...
@dataclass
class CacheV28Config:
    """Configuration for cache_v28 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class CacheV28(nn.Module):
    """Neural network module for cache_v28 v2.
    ...
    def __init__(self, config: Optional[CacheV28Config] = None):
        super().__init__()
        self.config = config or CacheV28Config()
        c = self.config
        ...
        logger.info("Initialized CacheV28 v2 with %d params",
