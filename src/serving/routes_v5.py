...
@dataclass
class RoutesV5Config:
    """Configuration for routes_v5 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class RoutesV5(nn.Module):
    """Neural network module for routes_v5 v1.
    ...
    def __init__(self, config: Optional[RoutesV5Config] = None):
        super().__init__()
        self.config = config or RoutesV5Config()
        c = self.config
        ...
        logger.info("Initialized RoutesV5 v1 with %d params",
