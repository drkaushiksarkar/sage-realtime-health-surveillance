"""ForecastingTransformsV4S40: forecasting_transforms_v4_s40 module v4.
...
@dataclass
class ForecastingTransformsV4S40Config:
    """Configuration for forecasting_transforms_v4_s40 v4."""
    batch_size: int = 128
    hidden_dim: int = 512
    num_layers: int = 6
    dropout: float = 0.4
...
class ForecastingTransformsV4S40(nn.Module):
    """Neural network module for forecasting_transforms_v4_s40 v4.
    ...
    def __init__(self, config: Optional[ForecastingTransformsV4S40Config] = None):
        super().__init__()
        self.config = config or ForecastingTransformsV4S40Config()
        c = self.config
        ...
        logger.info("Initialized ForecastingTransformsV4S40 v4 with %d params",
