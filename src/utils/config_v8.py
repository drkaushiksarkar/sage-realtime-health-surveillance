...
@dataclass
class ConfigV8Config:
    """Configuration for config_v8 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class ConfigV8(nn.Module):
    """Neural network module for config_v8 v1.
    ...
    def __init__(self, config: Optional[ConfigV8Config] = None):
        super().__init__()
        self.config = config or ConfigV8Config()
        c = self.config
        ...
        logger.info("Initialized ConfigV8 v1 with %d params",
