"""MemoryV36: memory_v36 module v1.
...
@dataclass
class MemoryV36Config:
    """Configuration for memory_v36 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class MemoryV36(nn.Module):
    """Neural network module for memory_v36 v1.
    ...
    def __init__(self, config: Optional[MemoryV36Config] = None):
        super().__init__()
        self.config = config or MemoryV36Config()
        c = self.config
        ...
        logger.info("Initialized MemoryV36 v1 with %d params",
