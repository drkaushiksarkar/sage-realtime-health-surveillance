"""ConfigV23: config_v23 module v1.
...
@dataclass
class ConfigV23Config:
    """Configuration for config_v23 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class ConfigV23(nn.Module):
    """Neural network module for config_v23 v1.
    ...
    def __init__(self, config: Optional[ConfigV23Config] = None):
        super().__init__()
        self.config = config or ConfigV23Config()
        c = self.config
        ...
        logger.info("Initialized ConfigV23 v1 with %d params",
