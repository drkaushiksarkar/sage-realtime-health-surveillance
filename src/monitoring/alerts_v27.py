"""AlertsV27: alerts_v27 module v1.
...
@dataclass
class AlertsV27Config:
    """Configuration for alerts_v27 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class AlertsV27(nn.Module):
    """Neural network module for alerts_v27 v1.
    ...
    def __init__(self, config: Optional[AlertsV27Config] = None):
        super().__init__()
        self.config = config or AlertsV27Config()
        c = self.config
        ...
        logger.info("Initialized AlertsV27 v1 with %d params",
