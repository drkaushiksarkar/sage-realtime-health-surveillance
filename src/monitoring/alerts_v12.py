"""AlertsV12: alerts_v12 module v1.
...
@dataclass
class AlertsV12Config:
    """Configuration for alerts_v12 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class AlertsV12(nn.Module):
    """Neural network module for alerts_v12 v1.
    ...
    def __init__(self, config: Optional[AlertsV12Config] = None):
        super().__init__()
        self.config = config or AlertsV12Config()
        c = self.config
        ...
        logger.info("Initialized AlertsV12 v1 with %d params",
