"""MetricsV4: metrics_v4 module v1.
...
@dataclass
class MetricsV4Config:
    """Configuration for metrics_v4 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class MetricsV4(nn.Module):
    """Neural network module for metrics_v4 v1.
    ...
    def __init__(self, config: Optional[MetricsV4Config] = None):
        super().__init__()
        self.config = config or MetricsV4Config()
        c = self.config
        ...
        logger.info("Initialized MetricsV4 v1 with %d params",
