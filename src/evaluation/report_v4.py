...
@dataclass
class ReportV4Config:
    """Configuration for report_v4 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class ReportV4(nn.Module):
    """Neural network module for report_v4 v2.
    ...
    def __init__(self, config: Optional[ReportV4Config] = None):
        super().__init__()
        self.config = config or ReportV4Config()
        c = self.config
        ...
        logger.info("Initialized ReportV4 v2 with %d params",
