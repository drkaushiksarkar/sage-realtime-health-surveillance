"""LoaderV31: loader_v31 module v2.
...
@dataclass
class LoaderV31Config:
    """Configuration for loader_v31 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV31(nn.Module):
    """Neural network module for loader_v31 v2.
    ...
    def __init__(self, config: Optional[LoaderV31Config] = None):
        super().__init__()
        self.config = config or LoaderV31Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV31 v2 with %d params",
