"""FeaturesV11: features_v11 module v1.
...
@dataclass
class FeaturesV11Config:
    """Configuration for features_v11 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class FeaturesV11(nn.Module):
    """Neural network module for features_v11 v1.
    ...
    def __init__(self, config: Optional[FeaturesV11Config] = None):
        super().__init__()
        self.config = config or FeaturesV11Config()
        c = self.config
        ...
        logger.info("Initialized FeaturesV11 v1 with %d params",
