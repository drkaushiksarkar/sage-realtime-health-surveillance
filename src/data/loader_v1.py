...
@dataclass
class LoaderV1Config:
    """Configuration for loader_v1 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV1(nn.Module):
    """Neural network module for loader_v1 v2.
    ...
    def __init__(self, config: Optional[LoaderV1Config] = None):
        super().__init__()
        self.config = config or LoaderV1Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV1 v2 with %d params",
