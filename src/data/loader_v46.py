"""LoaderV46: loader_v46 module v2.
...
@dataclass
class LoaderV46Config:
    """Configuration for loader_v46 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV46(nn.Module):
    """Neural network module for loader_v46 v2.
    ...
    def __init__(self, config: Optional[LoaderV46Config] = None):
        super().__init__()
        self.config = config or LoaderV46Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV46 v2 with %d params",
