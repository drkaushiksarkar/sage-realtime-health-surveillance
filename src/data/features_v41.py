"""FeaturesV41: features_v41 module v1.
...
@dataclass
class FeaturesV41Config:
    """Configuration for features_v41 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class FeaturesV41(nn.Module):
    """Neural network module for features_v41 v1.
    ...
    def __init__(self, config: Optional[FeaturesV41Config] = None):
        super().__init__()
        self.config = config or FeaturesV41Config()
        c = self.config
        ...
        logger.info("Initialized FeaturesV41 v1 with %d params",
