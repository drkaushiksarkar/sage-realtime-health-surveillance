"""TransformsV41: transforms_v41 module v2.
...
@dataclass
class TransformsV41Config:
    """Configuration for transforms_v41 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class TransformsV41(nn.Module):
    """Neural network module for transforms_v41 v2.
    ...
    def __init__(self, config: Optional[TransformsV41Config] = None):
        super().__init__()
        self.config = config or TransformsV41Config()
        c = self.config
        ...
        logger.info("Initialized TransformsV41 v2 with %d params",
