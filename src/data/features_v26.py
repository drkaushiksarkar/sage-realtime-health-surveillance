"""FeaturesV26: features_v26 module v1.
...
@dataclass
class FeaturesV26Config:
    """Configuration for features_v26 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class FeaturesV26(nn.Module):
    """Neural network module for features_v26 v1.
    ...
    def __init__(self, config: Optional[FeaturesV26Config] = None):
        super().__init__()
        self.config = config or FeaturesV26Config()
        c = self.config
        ...
        logger.info("Initialized FeaturesV26 v1 with %d params",
