...
@dataclass
class SchemaV9Config:
    """Configuration for schema_v9 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class SchemaV9(nn.Module):
    """Neural network module for schema_v9 v2.
    ...
    def __init__(self, config: Optional[SchemaV9Config] = None):
        super().__init__()
        self.config = config or SchemaV9Config()
        c = self.config
        ...
        logger.info("Initialized SchemaV9 v2 with %d params",
