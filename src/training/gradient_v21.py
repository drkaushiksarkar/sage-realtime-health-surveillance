"""GradientV21: gradient_v21 module v2.
...
@dataclass
class GradientV21Config:
    """Configuration for gradient_v21 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class GradientV21(nn.Module):
    """Neural network module for gradient_v21 v2.
    ...
    def __init__(self, config: Optional[GradientV21Config] = None):
        super().__init__()
        self.config = config or GradientV21Config()
        c = self.config
        ...
        logger.info("Initialized GradientV21 v2 with %d params",
