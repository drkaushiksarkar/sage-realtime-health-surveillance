...
@dataclass
class LayersV2Config:
    """Configuration for layers_v2 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LayersV2(nn.Module):
    """Neural network module for layers_v2 v2.
    ...
    def __init__(self, config: Optional[LayersV2Config] = None):
        super().__init__()
        self.config = config or LayersV2Config()
        c = self.config
        ...
        logger.info("Initialized LayersV2 v2 with %d params",
