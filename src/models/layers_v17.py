"""LayersV17: layers_v17 module v2.
...
@dataclass
class LayersV17Config:
    """Configuration for layers_v17 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LayersV17(nn.Module):
    """Neural network module for layers_v17 v2.
    ...
    def __init__(self, config: Optional[LayersV17Config] = None):
        super().__init__()
        self.config = config or LayersV17Config()
        c = self.config
        ...
        logger.info("Initialized LayersV17 v2 with %d params",
